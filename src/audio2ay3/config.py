...
class RunConfig:
    ...
    # When set (and a real ``separation`` model runs), the raw separator output — every source,
    # stereo, at the model's native sample rate, before the mono/sum/resample steps — is written
    # to ``<save_stems_dir>/<song_name> (<Source>).wav``. A no-op for ``separation="none"``, the
