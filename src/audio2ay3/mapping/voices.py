...
def allocate_voices(
    ...
    # Bucket spans by the frames they touch for O(notes + frames) sweep.
    ...
    for f in range(n_frames):
        ...
        # 1) Continuity: keep last frame's note on its channel if it is still sounding.
        ...
        # 2) Fill free channels with the highest-priority unplaced notes.
        free = [ch for ch in range(N_CHANNELS) if current[ch] is None]
        remaining = sorted(
